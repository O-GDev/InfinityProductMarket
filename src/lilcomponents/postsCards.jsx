...
export default function PostsCards({ image, opa, pname, pprice, jobtype, padin, name, ppic }) {
    return (
        <div className='flex flex-col bg-transparent border rounded-3xl shadow-md' >
            <img src={image} style={{ width: '100%', padding: padin }} className='' />
            <div className='flex-col bg-white self-end px-2 py-2 w-full h-full flex rounded-lg' style={{ opacity: opa, }} >
                <div className='font-semibold text-sm flex flex-row justify-between'>
                    <div className='text-sm' style={{ fontSize: "80%" }}> {pname} </div>
                    <div className='text-red-700' style={{fontSize:"80%"}}>N {pprice}</div> </div>
                <div >
                    <div className='flex flex-row justify-between pt-2'>
                        <div className='flex flex-row self-center'>
                            ...
                            <p className='self-center' style={{ fontSize: '60%', padding: 2 }}>{name} </p>
                        </div>
                        <div className='flex flex-row pl-2'>
                            <img src='/assets/Shop.png' style={{ width: '20px', height: '20px', }} className='flex justify-center self-center' />
                            <div className='self-center' style={{ fontSize: '60%' }}>Shop.{jobtype} </div>
                        </div>
                    </div>
